...
class TimeStrategy(BaseStrategy):
    """Improves on MetricStrategy by implementing time-dependent strategy.

    Features differrent implementation of calculating gamma value,
    instead of going back by discrete round counts it uses time it would
    take a robot to move to relevant cell, until time runs out.

    Time required for cell penetration is maybe a bit easier to figure out
    than discrete values, but will probably need manual adjustments both
    ...
    def gamma(self, h: int, w: int, i: int, j: int, probability=0) -> float:
        ...
        #     return gamma
        tau, prob = self.get_robot_strategy(i, j)
        ...
        if h - tau <= 0:
            ...
            for x in self.environment:
                if x.index != w:
                    current_gamma = self.gamma(h-tau, w, i, x.index, gamma)
                    _, current_alpha = self.get_robot_strategy(x.index, j)
                    gamma = gamma + current_gamma * current_alpha

            return gamma

    def get_robot_strategy(self, i: int, j: int) -> tuple[float, float]:
        """! Return a probability for the robot to go from cell `i` to `j`.

        Get sought probability and time it would take for the robot to go
        from cell 'i' to 'j' given the average velocity plus time it would
        take to search the cell

        @return tuple(time, probability) - to get from 'i' to 'j'
        """
    ...
    def get_time_to_travel(self, i, j):
        """Calculate how long it takes for robot to get from cell 'i' to 'j'.

        This will also be averaged like in dijkstra algorithm, take half
        of the size of both cells and calculate the time using average vel.

        The method assumes cell are adjacent to each other.

        @return time in seconds it takes to travel from 'i' to 'j'.
        """

        c1 = self.environment[i]
        c2 = self.environment[j]

        distance = (c1.dim + c2.dim) / 2
        return distance / self.p_velocity
